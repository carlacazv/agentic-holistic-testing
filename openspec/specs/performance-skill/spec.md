...
The skill SHALL capture Lighthouse performance evidence for declared pages, including raw reports, key metrics, run conditions, and repeated-run variability.
...
Budget mode SHALL evaluate each metric using its declared direction, threshold, and unit. Baseline comparison SHALL identify deltas but label regression only when a declared comparison rule supports it.
