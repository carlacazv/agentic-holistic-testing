...
Use the runtime commands instead of reconstructing state manually: run `doctor` before broad work, `cycle-start ... --output <file>` to create durable state, `cycle-resume` to select the next eligible skill, `cycle-complete` only after a checksum-valid run exists, and `cycle-summary` for a concise handoff. Generate or validate IDs with `run-id`; every run ID starts with `run-`. `cycle-start` creates missing parent directories, while `cycle-complete` rejects missing, invalid, or wrong-skill runs. Never mark a step complete without that validation.
