...
Only `automate` rows with explicit approval and recommended level API or browser E2E are derived as Playwright candidates from the canonical strategy. A recommendation is not approval. Record gaps and residual risks for deferred or unavailable coverage; never silently omit a plan case.

Required artifacts are `automation-strategy/strategy.json`, `automation-strategy/summary.md`, and foundation controls. Derive approved candidates on read; never persist a filtered second source. Metrics belong only in the return envelope. Return `completed` only at 100% assessed cases, `partial` for valid incomplete scope, and `blocked` when the upstream plan fails integrity validation.
